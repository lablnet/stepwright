...
def replace_index_placeholders(
    text: Optional[str], i: int, char: str = "i"
) -> Optional[str]:
    """Replace index placeholders ({{ char }}, {{ char_plus1 }}) in text"""
    if not text:
        return text
    # Convert to string if it's not already a string
    text_str = str(text) if not isinstance(text, str) else text
    return (
        text_str.replace(f"{{{{ {char} }}}}", str(i))
        .replace(f"{{{{{char}}}}}", str(i))
        .replace(f"{{{{ {char}_plus1 }}}}", str(i + 1))
        .replace(f"{{{{{char}_plus1}}}}", str(i + 1))
    )


def replace_data_placeholders(
    text: Optional[str], collector: Dict[str, Any]
) -> Optional[str]:
...
def flatten_nested_foreach_results(item: Dict[str, Any]) -> Any:
    ...
    """
    # Extract "context" keys (parent/global metadata).
    # These are keys that are not internal loop counters (item_0, item_1, etc.).
    # Saving these allows us to merge parent info (like a category name or page title)
    # into the results of child loops during the flattening process, ensuring
    # that every resulting record carries the necessary metadata from outer scopes.
    ...
    nested_item_keys = [k for k in item.keys() if re.match(r"^item_\d+$", k)]
    if nested_item_keys:
        ...
        for k in sorted(nested_item_keys, key=lambda x: int(x.split("_")[1])):
            ...
            if child_val:
                if isinstance(child_val, list):
                    # If the child is a list (nested loop), merge context into each element
                    for sub_item in child_val:
                        if isinstance(sub_item, dict):
                            flattened_items.append({**context, **sub_item})
                        else:
                            flattened_items.append(sub_item)
                elif isinstance(child_val, dict):
                    # Standard case: merge context into the dictionary
                    flattened_items.append({**context, **child_val})
                else:
                    flattened_items.append(child_val)
...
async def evaluate_condition(
    page: Page, condition: str, collector: Dict[str, Any]
) -> bool:
    """Evaluate JavaScript condition with collector data available"""
    try:
        # Replace collector placeholders in condition
        condition_with_data = (
            replace_data_placeholders(condition, collector) or condition
        )
...
async def apply_random_delay(
    page: Page, random_delay: Optional[Dict[str, int]]
) -> None:
...
def transform_data_regex(
    value: Any, regex: Optional[str], regex_group: Optional[int]
) -> Any:
...
async def apply_transform(
    page: Page, value: Any, transform: Optional[str], collector: Dict[str, Any]
) -> Any:
    ...
    try:
        ...
        transform_with_data = (
            replace_data_placeholders(transform, collector) or transform
        )
